...
func Test(t *testing.T) {
	...
	t.Run("GivenHealthyStatus_WhenHealth_ThenMatchSnapshot", func(t *testing.T) {
		...
		testFunc := func(requestHandler common.RequestHandler) {
			...
			utils.DoHTTPRequestWithResponse(t, requestHandler.Gin, request, &healthResponse, utils.ExpectHTTPStatus(http.StatusOK))

			snaps.MatchSnapshot(t, healthResponse)
		}

		healthChecker := fx.Provide(
			fx.Annotate(func() checker.HealthChecker { return healthChecker(t, checker.Healthy) }, fx.ResultTags(`group:"checkers"`)),
		)
	...
	t.Run("GivenUnhealthyStatus_WhenHealth_ThenMatchSnapshot", func(t *testing.T) {
		...
		testFunc := func(requestHandler common.RequestHandler) {
			request := utils.NewHTTPRequest(t, "GET", "/api/health/", nil)
			healthResponse := healthAPI.HealthResponse{}
			utils.DoHTTPRequestWithResponse(t, requestHandler.Gin, request, &healthResponse, utils.ExpectHTTPStatus(http.StatusInternalServerError))

			snaps.MatchSnapshot(t, healthResponse)
		}

		healthChecker := fx.Provide(
			fx.Annotate(func() checker.HealthChecker { return healthChecker(t, checker.Unhealthy) }, fx.ResultTags(`group:"checkers"`)),
		)
...
func healthChecker(t testing.TB, healthStatus checker.HealthStatus) checker.HealthChecker {
